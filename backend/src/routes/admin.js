import {
  findAdminByLogin, findAdminById, findAdminSession, listAdminAccounts, createAdminAccount,
  updateAdminLogin, touchAdminLogin, createAdminSession, revokeAdminSession, verifyPassword,
  listAdminAuditLogs, recordAdminAudit,
} from '../db.js';
import { adminRoles } from '../domain/validators.js';

/** @param {any} ctx */
export async function tryHandleAdmin(ctx) {
  const {
    request, response, path, url, body,
    db, adminSessionTtlMs, consumeRateLimit,
    send, httpError, assertRequired, assertMaxLengths, text, tokenHash, isoAfter,
    authenticateAdmin, requireAdminRole, adminBearer, adminAccount, assertAdminAccountMutation, hasOwn,
    randomBytes,
  } = ctx;
  ...
  if (request.method === 'POST' && path === '/admin/auth/login') {
    if (!consumeRateLimit(`admin-login:${request.socket.remoteAddress}`, 10, 15 * 60 * 1000)) {
      throw httpError(429, 'RATE_LIMITED', '登录尝试过于频繁，请稍后再试');
    }
    ...
    send(response, 200, { data: { token, expiresAt: isoAfter(adminSessionTtlMs), admin: adminAccount(current) } });
    return true;
  }
  ...
  if (request.method === 'GET' && path === '/admin/auth/me') {
    send(response, 200, { data: adminAccount(authenticateAdmin(request, db)) });
    return true;
  }
  ...
  if (request.method === 'GET' && path === '/admin/accounts') {
    ...
    send(response, 200, { data: listAdminAccounts(db) });
    return true;
  }
  ...
  if (request.method === 'POST' && path === '/admin/accounts') {
    ...
    if (admin.role !== 'owner' && text(body.role) === 'owner') {
      throw httpError(403, 'OWNER_PROTECTED', '非所有者不能创建所有者账号');
    }
    if (text(body.loginName).length > 120) throw httpError(422, 'VALIDATION_ERROR', '管理员账号长度无效');
    if (text(body.password).length < 12 || text(body.password).length > 256) {
      throw httpError(422, 'VALIDATION_ERROR', '管理员密码长度必须为 12 到 256 位');
    }
    try {
      send(response, 201, {
        data: createAdminAccount(db, {
          loginName: text(body.loginName),
      ...
      if (String(error.message).includes('UNIQUE constraint failed')) {
        throw httpError(409, 'ADMIN_EXISTS', '管理员账号已存在');
      }
      throw error;
    }
  }
  ...
  if (request.method === 'PATCH' && adminAccountPath) {
    const admin = authenticateAdmin(request, db);
    requireAdminRole(admin, ['owner', 'admin']);
    if (!['password', 'status', 'role'].some((field) => hasOwn(body, field))) {
      throw httpError(422, 'VALIDATION_ERROR', '至少需要修改一个管理员字段');
    }
    if (hasOwn(body, 'status') && !['active', 'disabled'].includes(text(body.status))) {
      throw httpError(422, 'VALIDATION_ERROR', '管理员状态无效');
    }
    if (hasOwn(body, 'role') && !adminRoles.has(text(body.role))) {
      throw httpError(422, 'VALIDATION_ERROR', '管理员角色无效');
    }
    if (hasOwn(body, 'password') && (text(body.password).length < 12 || text(body.password).length > 256)) {
      throw httpError(422, 'VALIDATION_ERROR', '管理员密码长度必须为 12 到 256 位');
    }
    ...
    send(response, 200, { data: updated });
    return true;
  }
  ...
  if (request.method === 'GET' && path === '/admin/audit-logs') {
    ...
    if (!Number.isInteger(limit) || limit < 1 || limit > 200) {
      throw httpError(422, 'VALIDATION_ERROR', '审计日志数量限制无效');
    }
    send(response, 200, { data: listAdminAuditLogs(db, limit) });
    return true;
  }
