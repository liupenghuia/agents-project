import {
  ...
  getApplicantJobSeekingInformation, mapApplicantJobSeekingInformation, upsertApplicantJobSeekingInformation,
  getRecruiterInformation, mapRecruiterInformation, upsertRecruiterInformation,
  createMediaUpload, completeMediaUpload,
  listRecruitmentPosts, createRecruitmentPost, getRecruitmentPost, updateRecruitmentPost,
  setMarketVisibility, renewMarketPublication,
} from '../db.js';
import {
  imageTypes, maxImageBytes,
  assertJobSeekingInformation, assertRecruiterInformation, assertRecruitmentPost,
...
export async function tryHandleInformation(ctx) {
  const {
    request, response, path, body, uploadPath,
    db, mediaRoot,
    send, httpError, assertRequired, assertMaxLengths, text, isoAfter,
    authenticate, randomBytes, mkdir, writeFile, join,
  } = ctx;
  ...
  if (request.method === 'POST' && path === '/me/recruitment-posts/image-upload-url') {
    ...
    if (!findRoleProfileForUser(db, user.id, 'recruiter')) {
      throw httpError(403, 'IDENTITY_REQUIRED', '需要先创建招人身份');
    }
    ...
    send(response, 200, {
      data: { objectKey, uploadUrl: `/me/recruitment-posts/image-upload/${objectKey}`, expiresAt },
    });
    return true;
  }
  ...
  if (request.method === 'POST' && uploadPath) {
    ...
    send(response, 200, {
      data: { objectKey: upload.object_key, contentType: upload.content_type, byteSize: file.length },
    });
    return true;
  }
  ...
  if (request.method === 'GET' && path === '/me/applicant/job-seeking-information') {
    ...
    send(response, 200, { data: information && mapApplicantJobSeekingInformation(information) });
    return true;
  }
  ...
  if (request.method === 'PUT' && path === '/me/applicant/job-seeking-information') {
    const user = authenticate(request, db);
    send(response, 200, {
      data: mapApplicantJobSeekingInformation(
        upsertApplicantJobSeekingInformation(db, user.id, assertJobSeekingInformation(body)),
      ),
    });
    return true;
  }
  ...
  if (request.method === 'GET' && path === '/me/recruiter/information') {
    ...
    send(response, 200, { data: information && mapRecruiterInformation(information) });
    return true;
  }
  ...
  if (request.method === 'PUT' && path === '/me/recruiter/information') {
    const user = authenticate(request, db);
    send(response, 200, {
      data: mapRecruiterInformation(upsertRecruiterInformation(db, user.id, assertRecruiterInformation(body))),
    });
    return true;
  }
  ...
  if (request.method === 'GET' && path === '/me/recruitment-posts') {
    const user = authenticate(request, db);
    send(response, 200, { data: listRecruitmentPosts(db, user.id) });
    return true;
  }

  if (request.method === 'POST' && path === '/me/recruitment-posts') {
    const user = authenticate(request, db);
    send(response, 201, { data: createRecruitmentPost(db, user.id, assertRecruitmentPost(body)) });
    return true;
  }
  ...
  if (request.method === 'PATCH' && recruitmentPostPath) {
    ...
    send(response, 200, { data: updateRecruitmentPost(db, user.id, recruitmentPostPath[1], next) });
    return true;
  }

  if (request.method === 'POST' && disablePostPath) {
    const user = authenticate(request, db);
    if (!setMarketVisibility(db, user.id, 'recruitment_post', disablePostPath[1], false)) {
  ...
  if (request.method === 'POST' && path === '/me/applicant/job-seeking-information/disable') {
    const user = authenticate(request, db);
    if (!setMarketVisibility(db, user.id, 'applicant_information', '', false)) {
  ...
  if (request.method === 'POST' && path === '/me/applicant/job-seeking-information/renew') {
    ...
    send(response, 200, { data: mapApplicantJobSeekingInformation(information) });
    return true;
  }
